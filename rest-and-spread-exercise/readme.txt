...
const findMin = (...args) => args.reduce((min, val) => min < val ? min : val);
...
const doubleAndReturnArgs = (arr, ...theRest) => [...arr, ...theRest.map((val) => val * 2)];
...
const removeRandom = (...items) => {
    ...
    return items.filter((val, idx) => idx !== rnd ? true : false);
}
